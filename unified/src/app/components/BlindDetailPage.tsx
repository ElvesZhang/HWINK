 ...
 *  extra: Time leads. The raw data is a `flow` field so any length pages without
 *  clipping. */
export function BlindDetailPage({ onBack, detail }: BlindDetailPageProps) {
  const fields: DetailField[] = [
    ...
    { label: 'Method ID', value: <div className="text-xl font-normal text-black break-all">{detail.methodId}</div> },
    { label: 'Contract', value: <BoldEndsAddress addr={detail.contractAddress} className="text-xl text-black break-all leading-snug" /> },
    {
      label: 'Raw Data',
      flow: true,
      value: <div className="text-xl font-normal text-black break-all leading-snug">{detail.rawData}</div>,
    },
  ];
